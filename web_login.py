...
if __name__ == '__main__':
    target_script = 'flowborn_poster.py'
    args = sys.argv[1:]
    if len(sys.argv) > 1 and sys.argv[1] == 'loadtran':
        target_script = 'loadtran.py'
        args = sys.argv[2:]

    # When run directly, check license then exec main tool
    if '--help' in args or '-h' in args:
        os.execvp(sys.executable, [sys.executable, target_script] + args)
